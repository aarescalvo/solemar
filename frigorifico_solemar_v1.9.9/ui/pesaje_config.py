"""
Pesaje - Configuración
"""
import tkinter as tk
from tkinter import messagebox
from datetime import datetime
from core import theme as T
from core.database import get_config_tickets, set_config_ticket, autenticar_usuario
...
class PesajeConfig(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(header, text="⚙️ CONFIGURACIÓN DE TICKETS", font=T.FONT_SUBTITULO, bg=T.GRIS_OSCURO, fg=T.TEXTO_CLARO).pack(side="left", padx=15, pady=10)

        content = tk.Frame(self, bg=T.FONDO)
        content.pack(expand=True, fill="both", padx=50, pady=30)

        tk.Label(content, text="Numeración de Tickets", font=T.FONT_TITULO, bg=T.FONDO, fg=T.TEXTO).pack(pady=20)

        config_frame = tk.Frame(content, bg=T.BLANCO, relief="solid", bd=1)
        config_frame.pack(pady=20, padx=50, fill="x")
        ...
        tk.Label(row1, text="📥 Próximo N° Ticket INGRESO:", font=T.FONT_BOTON, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=10)
        ...
        tk.Button(row1, text="✏️ Modificar", font=T.FONT_BOTON, bg=T.AZUL, fg=T.TEXTO_CLARO, relief="flat", cursor="hand2",
                  command=lambda: self._modificar("ingreso")).pack(side="left", padx=15)

        row2 = tk.Frame(config_frame, bg=T.BLANCO)
        row2.pack(fill="x", padx=20, pady=15)

        tk.Label(row2, text="📤 Próximo N° Ticket EGRESO:", font=T.FONT_BOTON, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=10)
        ...
        tk.Button(row2, text="✏️ Modificar", font=T.FONT_BOTON, bg=T.AZUL, fg=T.TEXTO_CLARO, relief="flat", cursor="hand2",
                  command=lambda: self._modificar("egreso")).pack(side="left", padx=15)

        tk.Label(config_frame, text="ℹ️ Para modificar se requiere clave de supervisor", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.GRIS_OSCURO).pack(pady=15)

        preview = tk.Frame(content, bg=T.AZUL)
        preview.pack(pady=20, padx=50, fill="x")

        tk.Label(preview, text="Vista Previa de Próximos Tickets", font=T.FONT_SUBTITULO, bg=T.AZUL, fg=T.TEXTO_CLARO).pack(pady=10)
    ...
    def _modificar(self, tipo):
        ...
        dlg.title("Autorización Requerida")
        ...
        tk.Label(dlg, text="🔐 AUTORIZACIÓN DE SUPERVISOR", font=T.FONT_SUBTITULO, bg=T.FONDO, fg=T.ROJO).pack(pady=20)

        frame = tk.Frame(dlg, bg=T.BLANCO, relief="solid", bd=1)
        frame.pack(pady=10, padx=30, fill="x")

        tk.Label(frame, text="Usuario:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).grid(row=0, column=0, padx=10, pady=8, sticky="e")
        entry_user = tk.Entry(frame, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, relief="solid", bd=1, width=20)
        entry_user.grid(row=0, column=1, padx=10, pady=8)

        tk.Label(frame, text="Contraseña:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).grid(row=1, column=0, padx=10, pady=8, sticky="e")
        entry_pass = tk.Entry(frame, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, relief="solid", bd=1, width=20, show="•")
        ...
        def autorizar():
            ...
            if not user or not pwd:
                messagebox.showwarning("Error", "Complete usuario y contraseña", parent=dlg)
                return
            ...
            if auth['rol'] not in ('supervisor', 'administrador'):
    ...
    def _editar(self, tipo):
        ...
        def guardar():
            try:
                ...
                dlg.destroy()
                messagebox.showinfo("Éxito", f"Ticket de {tipo} actualizado")

            except ValueError as e:
                messagebox.showerror("Error", f"Valor inválido: {e}", parent=dlg)
