...
class PesajeEgreso(tk.Frame):
    ...
    def _show_form(self):
        ...
        # COLUMNA IZQUIERDA - Info del ingreso
        ...
        # Observaciones
        obs_frame = tk.LabelFrame(left_frame, text=" OBSERVACIONES ", 
                                  font=T.FONT_SUBTITULO, bg=T.BLANCO, fg=T.AZUL, relief="solid", bd=1)
        obs_frame.pack(fill="both", expand=True, pady=8)

        self.text_obs = tk.Text(obs_frame, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                 fg=T.TEXTO, height=6, width=40, relief="solid", bd=1)
