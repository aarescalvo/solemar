"""
Pesaje - Configuracion v2.0.0
Corregido: usa autenticar_operador en lugar de autenticar_usuario
"""
...
class PesajeConfig(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(header, text="CONFIGURACION DE TICKETS", font=T.FONT_SUBTITULO, 
        ...
        tk.Label(content, text="Numeracion de Tickets", font=T.FONT_TITULO, 
                 bg=T.FONDO, fg=T.TEXTO).pack(pady=15)
        ...
        tk.Label(row1, text="Proximo N Ticket INGRESO:", font=T.FONT_BOTON, 
                 bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=10)

        self.entry_ingreso = tk.Entry(row1, font=T.FONT_MONO, bg=T.GRIS_CLARO, 
        ...
        tk.Label(row2, text="Proximo N Ticket EGRESO:", font=T.FONT_BOTON, 
                 bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=10)

        self.entry_egreso = tk.Entry(row2, font=T.FONT_MONO, bg=T.GRIS_CLARO, 
                                     fg=T.TEXTO, width=10, justify="right", relief="solid", bd=1)
        ...
        preview.pack(pady=15, padx=30, fill="x")

        tk.Label(preview, text="Vista Previa de Proximos Tickets", font=T.FONT_SUBTITULO, 
                 bg=T.AZUL, fg=T.TEXTO_CLARO).pack(pady=10)

        anio = datetime.now().year
        self.lbl_preview_in = tk.Label(preview, text=f"Ingreso: TI-{anio}-{cfg['prox_ingreso']:06d}", 
                                       font=T.FONT_MONO, bg=T.AZUL, fg=T.TEXTO_CLARO)
        self.lbl_preview_in.pack(pady=5)

        self.lbl_preview_out = tk.Label(preview, text=f"Egreso: TE-{anio}-{cfg['prox_egreso']:06d}", 
                                        font=T.FONT_MONO, bg=T.AZUL, fg=T.TEXTO_CLARO)
