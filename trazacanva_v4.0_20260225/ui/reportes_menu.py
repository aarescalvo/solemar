...
class ReportesMenu(tk.Frame):
    ...
    def _show_main_menu(self):
        ...
        opciones = [
            ...
            ("📋 MOV. CORRALES", "Movimientos de corrales", pipeline(self._show_mov_corrales), T.AZUL),
            ("📝 PLANILLA 01", "Ingreso de Hacienda", pipeline(self._show_planilla01), T.VERDE),
        ]
    ...
    def _show_mov_corrales(self):
        self._clear_current()
        from core.database import listar_movimientos_corral
        self._show_tabla("MOVIMIENTOS DE CORRALES", listar_movimientos_corral,
                         ["fecha", "hora", "animal", "origen", "destino", "motivo"],
                         {"fecha": ("Fecha", 100), "hora": ("Hora", 70), "animal": ("Animal", 120),
                          "origen": ("Origen", 80), "destino": ("Destino", 80), "motivo": ("Motivo", 180)})
    
    def _show_planilla01(self):
        self._clear_current()
        from core.database import listar_tickets_ingreso
        def adapt():
            datos = []
            for t in listar_tickets_ingreso():
                datos.append({
                    "fecha": f"{t.get('fecha','')} {t.get('hora','')}",
                    "productor": t.get("productor_nombre") or t.get("proveedor_nombre", "-"),
                    "procedencia": t.get("procedencia_lugar", "-"),
                    "renspa": t.get("renspa", "-"),
                    "tipo_faena": t.get("tipo_faena", "-"),
                    "patente": f"{t.get('patente_chasis','')} {t.get('patente_acoplado','')}",
                    "transportista": t.get("transportista","-"),
                    "chofer": t.get("chofer","-"),
                    "guia": t.get("numero_guia","-"),
                    "dte": t.get("numero_dte","-"),
                    "cabezas": t.get("cant_cabezas",0),
                    "muertos": t.get("cant_muertos",0),
                    "destino": t.get("destino_comercial","-"),
                    "bruto": t.get("peso_bruto_kg",0),
                    "tara": t.get("peso_tara_kg",0),
                    "neto": t.get("peso_neto_kg",0),
                })
            return datos
        self._show_tabla("PLANILLA 01 - INGRESO DE HACIENDA", adapt,
                         ["fecha","productor","procedencia","renspa","tipo_faena","patente","transportista","chofer","guia","dte","cabezas","muertos","destino","bruto","tara","neto"],
                         {"fecha":("Fecha/Hora",120),"productor":("Productor",160),"procedencia":("Procedencia",120),
                          "renspa":("RENSPA",90),"tipo_faena":("Tipo Faena",100),"patente":("Patente",110),
                          "transportista":("Transportista",150),"chofer":("Chofer",120),"guia":("Guía",90),
                          "dte":("DTE",80),"cabezas":("Cabezas",70),"muertos":("Muertos",70),"destino":("Destino",100),
                          "bruto":("Bruto",80),"tara":("Tara",80),"neto":("Neto",80)})
