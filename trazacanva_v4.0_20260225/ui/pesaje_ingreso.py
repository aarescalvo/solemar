...
from core.database import (
    ...
    buscar_transportista_by_patente, listar_tipos_faena
)
...
class PesajeIngreso(tk.Frame):
    ...
    def _show_form(self):
        ...
        tipos_faena = listar_tipos_faena() or ["UE", "No UE", "No Rusia", "Servicio de Faena"]
        ...
        # Fila 2: N Guia y N DTE y Fecha Emisión
        row = tk.Frame(form_left, bg=T.BLANCO)
        row.pack(fill="x", pady=4)
        ...
        tk.Label(row, text="F. Emisión:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=10, anchor="e").pack(side="left", padx=3)
        self.entry_femision = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                       width=12, relief="solid", bd=1)
        self.entry_femision.pack(side="left", padx=3)
        ...
        tk.Label(row, text="Tara (kg):", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=10, anchor="e").pack(side="left", padx=3)
        self.entry_tara = tk.Entry(row, font=T.FONT_MONO_SMALL, bg=T.GRIS_CLARO, 
                                   width=10, relief="solid", bd=1, justify="right")
        self.entry_tara.pack(side="left", padx=3)
        ...
        # Fila 9: DTE/Procedencia extra
        row = tk.Frame(form_left, bg=T.BLANCO)
        row.pack(fill="x", pady=4)
        tk.Label(row, text="Productor:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=14, anchor="e").pack(side="left", padx=3)
        self.entry_productor = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                        width=22, relief="solid", bd=1)
        self.entry_productor.pack(side="left", padx=3)
        tk.Label(row, text="Procedencia:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=12, anchor="e").pack(side="left", padx=3)
        self.entry_procedencia = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                          width=18, relief="solid", bd=1)
        self.entry_procedencia.pack(side="left", padx=3)
        row = tk.Frame(form_left, bg=T.BLANCO)
        row.pack(fill="x", pady=4)
        tk.Label(row, text="Renspa:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=14, anchor="e").pack(side="left", padx=3)
        self.entry_renspa = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                     width=14, relief="solid", bd=1)
        self.entry_renspa.pack(side="left", padx=3)
        tk.Label(row, text="Tipo Faena:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=12, anchor="e").pack(side="left", padx=3)
        self.combo_tipo_faena = ttk.Combobox(row, values=tipos_faena, width=18, state="readonly", font=T.FONT_NORMAL)
        self.combo_tipo_faena.pack(side="left", padx=3)
        row = tk.Frame(form_left, bg=T.BLANCO)
        row.pack(fill="x", pady=4)
        tk.Label(row, text="Tipo Proced.:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=14, anchor="e").pack(side="left", padx=3)
        self.combo_tipo_proc = ttk.Combobox(row, values=["Directo de Acopio", "Campo"], width=18, state="readonly", font=T.FONT_NORMAL)
        self.combo_tipo_proc.pack(side="left", padx=3)
        tk.Label(row, text="Destino Comercial:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=16, anchor="e").pack(side="left", padx=3)
        self.entry_destino_com = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                          width=18, relief="solid", bd=1)
        self.entry_destino_com.pack(side="left", padx=3)
        row = tk.Frame(form_left, bg=T.BLANCO)
        row.pack(fill="x", pady=4)
        tk.Label(row, text="Cabezas:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=14, anchor="e").pack(side="left", padx=3)
        self.entry_cabezas = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                      width=8, relief="solid", bd=1, justify="right")
        self.entry_cabezas.pack(side="left", padx=3)
        tk.Label(row, text="Muertos:", font=T.FONT_LABEL, bg=T.BLANCO, 
                 fg=T.TEXTO, width=10, anchor="e").pack(side="left", padx=3)
        self.entry_muertos = tk.Entry(row, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, 
                                      width=8, relief="solid", bd=1, justify="right")
        self.entry_muertos.pack(side="left", padx=3)
        ...
        tk.Button(peso_btns, text="Capturar Bruto", font=T.FONT_BOTON, bg=T.AZUL, 
                  fg=T.TEXTO_CLARO, relief="flat", cursor="hand2", padx=12, pady=5,
                  command=self._capturar_peso).pack(side="left", padx=8)
        tk.Button(peso_btns, text="Capturar Tara", font=T.FONT_BOTON, bg=T.AZUL, 
                  fg=T.TEXTO_CLARO, relief="flat", cursor="hand2", padx=12, pady=5,
                  command=self._capturar_tara).pack(side="left", padx=8)
    ...
    def _capturar_peso(self):
        ...
        import random
        peso = random.randint(12000, 30000)
        self.lbl_peso.configure(text=f"{peso:,.0f} kg")
        self.entry_peso.delete(0, tk.END)
        self.entry_peso.insert(0, str(peso))
        messagebox.showinfo("Balanza", f"Peso bruto capturado (simulado): {peso} kg\n\nConfigure la balanza en Configuracion > Equipos")

    def _capturar_tara(self):
        # Simulación de tara
        import random
        tara = random.randint(4000, 9000)
        try:
            self.entry_tara.delete(0, tk.END)
            self.entry_tara.insert(0, str(tara))
        except:
            pass
    ...
    def _guardar(self):
        ...
        datos = {
            ...
            "productor_nombre": self.entry_productor.get().strip(),
            "procedencia_lugar": self.entry_procedencia.get().strip(),
            "renspa": self.entry_renspa.get().strip(),
            "tipo_faena": self.combo_tipo_faena.get().strip(),
            "fecha_emision_guia": self.entry_femision.get().strip(),
            "tipo_procedencia": self.combo_tipo_proc.get().strip(),
            "cant_cabezas": int(self.entry_cabezas.get() or "0"),
            "cant_muertos": int(self.entry_muertos.get() or "0"),
            "destino_comercial": self.entry_destino_com.get().strip(),
            "precintos": self.entry_precintos.get().strip(),
            "observaciones": self.entry_obs.get("1.0", tk.END).strip(),
            "peso_kg": peso,
            "peso_bruto_kg": peso,
            "peso_tara_kg": float(self.entry_tara.get() or 0),
            "tara_camion_kg": float(self.entry_tara.get() or 0),
            "peso_manual": 1,
