...
from core.database import (
    ...
    # Tickets
    listar_tickets_ingreso_cerrados_no_vinculados, get_ticket_by_id, vincular_ticket_a_tropa,
    # Tipificaciones
...
class Paso1NuevaTropa(tk.Frame):
    ...
    def _build(self):
        ...
        cols = ("ticket", "fecha", "patente", "proveedor", "guia", "dte", "peso", "estado", "observaciones")
        ...
        self.tree_tickets.column("estado", width=80)
        ...
        tk.Button(ticket_frame, text="✅ Aceptar y continuar", font=T.FONT_LABEL, bg=T.VERDE, fg=T.TEXTO_CLARO,
                  relief="flat", cursor="hand2", command=self._siguiente_paso).pack(pady=(4, 10))
        ...
        tk.Button(btn_frame, text="✅ ACEPTAR Y CONTINUAR", font=T.FONT_BOTON, bg=T.VERDE, 
    ...
    def _cargar_tickets(self):
        ...
        tickets = listar_tickets_ingreso_cerrados_no_vinculados()
        self.tickets_map = {}
        ...
        for t in tickets:
            ...
            item_id = self.tree_tickets.insert("", "end", values=(
                ...
                t.get('estado', 'cerrado') or 'cerrado',
    ...
    def _siguiente_paso(self):
        ...
        try:
            ...
            # Vincular ticket a la tropa creada
            vincular_ticket_a_tropa(self.ticket_sel['id'], tropa_id)
...
class Paso3CapturaIndividual(tk.Frame):
    ...
    def _build(self):
        ...
        # Campos adicionales recepción
        extra1 = tk.Frame(left_frame, bg=T.BLANCO)
        extra1.pack(fill="x", padx=15, pady=10)
        tk.Label(extra1, text="Tipo de Animal:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=5)
        self.entry_tipo_an = tk.Entry(extra1, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, width=18, relief="solid", bd=1)
        self.entry_tipo_an.pack(side="left", padx=10)
        tk.Label(extra1, text="Color:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=5)
        self.entry_color = tk.Entry(extra1, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, width=12, relief="solid", bd=1)
        self.entry_color.pack(side="left", padx=10)

        extra2 = tk.Frame(left_frame, bg=T.BLANCO)
        extra2.pack(fill="x", padx=15, pady=10)
        tk.Label(extra2, text="Estado:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=5)
        self.combo_estado_rep = ttk.Combobox(extra2, values=["Normal", "Preñada"], width=12, state="readonly", font=T.FONT_NORMAL)
        self.combo_estado_rep.pack(side="left", padx=10)
        tk.Label(extra2, text="Destino:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=5)
        self.combo_destino = ttk.Combobox(extra2, values=["UE", "No UE", "No Rusia", "Servicio"], width=14, state="readonly", font=T.FONT_NORMAL)
        self.combo_destino.pack(side="left", padx=10)

        extra3 = tk.Frame(left_frame, bg=T.BLANCO)
        extra3.pack(fill="x", padx=15, pady=10)
        tk.Label(extra3, text="Desbaste (kg):", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=5)
        self.entry_desbaste = tk.Entry(extra3, font=T.FONT_MONO_SMALL, bg=T.GRIS_CLARO, width=8, relief="solid", bd=1, justify="right")
        self.entry_desbaste.pack(side="left", padx=10)
        tk.Label(extra3, text="Chip:", font=T.FONT_LABEL, bg=T.BLANCO, fg=T.TEXTO).pack(side="left", padx=5)
        self.entry_chip = tk.Entry(extra3, font=T.FONT_NORMAL, bg=T.GRIS_CLARO, width=18, relief="solid", bd=1)
        self.entry_chip.pack(side="left", padx=10)
    ...
    def _guardar(self, imprimir=True):
        ...
        datos = {
            ...
            "tipo_animal": self.entry_tipo_an.get().strip(),
            "color": self.entry_color.get().strip(),
            "estado_reproductivo": self.combo_estado_rep.get().strip(),
            "destino": self.combo_destino.get().strip(),
            "desbaste": float(self.entry_desbaste.get() or 0),
